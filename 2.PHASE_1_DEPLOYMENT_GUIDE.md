...
1.  **Di chuyển vào thư mục dự án:**
    - Thay `Ads-auto` bằng tên thư mục dự án của bạn nếu khác.
    ```bash
    cd Ads-auto
    ```
...
3.  **Cấu hình Biến môi trường cho Backend:**
    ...
    -   Điền các thông tin bạn đã tạo ở các bước trước:
        ...
        ADS_API_CLIENT_ID=your_ads_api_client_id
        ADS_API_CLIENT_SECRET=your_ads_api_client_secret
        ADS_API_REFRESH_TOKEN=your_ads_api_refresh_token
        # ... điền tất cả các giá trị còn lại ...
        ```
    -   Sau khi chỉnh sửa xong, nhấn `Ctrl + X`, sau đó `Y` và `Enter` để lưu lại.
    -   Quay lại thư mục gốc của dự án: `cd ..`
...
-   Từ thư mục gốc của dự án, chạy lệnh build:
    ```bash
    npm run build
    ```
...
1.  **Tạo file cấu hình mới cho trang web của bạn:**
    ...
    ```
2.  **Dán nội dung sau vào file (thay thế nội dung cũ):**
    -   **LƯU Ý QUAN TRỌNG:**
        -   Thay `your_vps_ip` bằng địa chỉ IP công khai của VPS của bạn.
        -   Thay `/home/yourusername/Ads-auto/dist` bằng đường dẫn **CHÍNH XÁC** đến thư mục `dist` của bạn.
    ```nginx
    server {
        listen 80;
        server_name your_vps_ip; # Hoặc tên miền của bạn nếu có

        # Đường dẫn đến thư mục build của frontend
        root /home/yourusername/Ads-auto/dist;
        index index.html;
...
### Bước 9: Chạy Backend với PM2

1.  **Di chuyển vào thư mục backend:**
    ```bash
    cd backend
    ```
2.  **Khởi động server bằng PM2:**
    ```bash
    pm2 start server.js --name "ppc-auto-backend"
    ```
...
4.  **Kiểm tra trạng thái:**
    ...
    pm2 list
    ```
    -   Bạn sẽ thấy `ppc-auto-backend` có trạng thái là `online`.

---

## Hoàn tất!

Bây giờ bạn có thể mở trình duyệt và truy cập vào **http://your_vps_ip**. Bạn sẽ thấy giao diện ứng dụng của mình. Giai đoạn 1 đã hoàn thành!

**Các lệnh PM2 cần nhớ:**
-   Xem logs (để kiểm tra lỗi): `pm2 logs ppc-auto-backend`
-   Khởi động lại ứng dụng sau khi thay đổi code: `pm2 restart ppc-auto-backend`
-   Dừng ứng dụng: `pm2 stop ppc-auto-backend`