...
3.  **Chạy lệnh SQL tạo bảng:** Sau khi vào được giao diện của `psql` (dấu nhắc sẽ đổi thành `amazon_data_analyzer=#`), hãy sao chép (copy) **CHÍNH XÁC** nội dung SQL bên trong hộp dưới đây (bắt đầu từ dòng `-- Bảng lưu trữ...`), sau đó dán (paste) vào terminal và nhấn Enter.
