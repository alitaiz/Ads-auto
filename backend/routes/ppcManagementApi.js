...
/**
 * Encapsulates the logic to create a full SP Auto campaign from an ASIN.
 * This function is exported so it can be used by both the API endpoint and the rules engine.
 * @param {string} profileId - The Amazon Ads profile ID.
 * @param {string} asin - The product ASIN.
 * @param {number} budget - The daily budget.
 * @param {number} defaultBid - The default ad group bid.
 * @param {number[]} associatedRuleIds - An array of automation rule IDs to associate.
 * @returns {Promise<object>} - An object with details of the created entities.
 */
export async function createAutoCampaign(profileId, asin, budget, defaultBid, associatedRuleIds = []) {
    let client;
    try {
        console.log(`[Action:CreateAutoCampaign] Starting creation for ASIN ${asin}`);
        // 1. Get SKU from ASIN
        const sku = await getSkuByAsin(asin);
        if (!sku) {
            throw new Error(`Could not find a valid SKU for ASIN ${asin}. Add it to your Listings or ensure it's in Seller Central.`);
        }
        console.log(`[Action:CreateAutoCampaign] Found SKU: ${sku}`);
        ...
        const campaignPayload = {
            ...
            targetingType: 'AUTO',
            state: 'ENABLED',
            budget: { budget: Number(budget), budgetType: 'DAILY' },
        ...
        if (!campSuccess?.campaignId) {
            const errorDetail = campResponse?.campaigns?.error?.[0]?.errors?.[0]?.errorValue?.message;
            throw new Error(`Campaign creation failed: ${errorDetail || JSON.stringify(campResponse?.campaigns?.error?.[0])}`);
        }
        const newCampaignId = campSuccess.campaignId;
        console.log(`[Action:CreateAutoCampaign] Created Campaign ID: ${newCampaignId}`);
        ...
        console.log(`[Action:CreateAutoCampaign] Created Ad Group ID: ${newAdGroupId}`);
        ...
        console.log(`[Action:CreateAutoCampaign] Created Product Ad ID: ${adSuccess.adId}`);

        // 5. Associate Rules
        let rulesAssociated = 0;
        if (associatedRuleIds && associatedRuleIds.length > 0) {
            console.log(`[Action:CreateAutoCampaign] Associating ${associatedRuleIds.length} rules...`);
            ...
            for (const ruleId of associatedRuleIds) {
                ...
                if (rows.length > 0) {
                    ...
                    campaignIds.add(String(newCampaignId));
            ...
            console.log(`[Action:CreateAutoCampaign] Associated ${rulesAssociated} rules.`);
        }
        ...
        console.error('[Action:CreateAutoCampaign] Error during campaign creation:', error);
        throw error;
    } finally {
...
router.post('/campaigns/list', async (req, res) => {
    ...
    try {
        ...
        const [spCampaigns, sbCampaigns, sdCampaigns] = await Promise.all([spPromise, sbCampaigns, sdCampaigns]);
...
router.post('/create-auto-campaign', async (req, res) => {
    ...
    try {
        const result = await createAutoCampaign(profileId, asin, budget, defaultBid, ruleIds);
        res.status(201).json({ 
