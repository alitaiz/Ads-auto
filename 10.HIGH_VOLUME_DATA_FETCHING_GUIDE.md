...
-   Bạn đã chạy file migration `005_add_report_requests_table.sql.txt` để tạo bảng `report_requests`. Bảng này đóng vai trò là hàng đợi (queue) để theo dõi các yêu cầu. Chạy lệnh sau trong `psql` nếu bạn chưa làm:
    ```sql
    \i /path/to/your/project/backend/migrations/005_add_report_requests_table.sql.txt
    ```
...
2.  **Chạy script với khoảng thời gian bạn muốn backfill.**
    ...
    ```bash
    npm run request:sp-search-terms -- 2024-08-11 2024-10-10
    ```
...
2.  **Thêm dòng sau vào cuối file.**
    ...
    ```cron
    # Tự động xử lý các báo cáo đang chờ trong hàng đợi mỗi 15 phút
    */15 * * * * cd /var/www/Ads-auto && npm run process:pending-reports >> /var/www/Ads-auto/logs/process_pending_reports.log 2>&1
    ```
