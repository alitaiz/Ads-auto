...
const styles: { [key: string]: React.CSSProperties } = {
    ...
    title: { fontSize: '2rem', marginBottom: '0', textAlign: 'center' },
...
export function CreateAdsView() {
    ...
    const fetchSchedulesAndRules = useCallback(async () => {
        ...
        try {
            ...
            if (rulesRes.ok) {
                const allRulesData: AutomationRule[] = await rulesRes.json();
                setAllRules(allRulesData.filter(rule => rule.profile_id === profileId));
            }
            if (schedulesRes.ok) {
                const schedulesData: CampaignCreationRule[] = await schedulesRes.json();
    ...
    const categorizedRules = useMemo(() => {
        return allRules.reduce((acc, rule) => {
            ...
            if (type === 'PRICE_ADJUSTMENT') return acc;
    ...
    const handleScheduleStatusToggle = async (schedule: CampaignCreationRule) => {
        ...
        try {
            const response = await fetch(`/api/automation/campaign-creation-rules/${schedule.id}`, {
                ...
                body: JSON.stringify({ is_active: updatedSchedule.is_active }),
            });
        ...
        try {
            ...
            if (isScheduling) {
                const schedulePayload = {
                    ...
                    is_active: true,
                    frequency,
                    creation_parameters: commonPayload,
            ...
            if (isScheduling) {
                setStatusMessage({ type: 'success', text: `Successfully created schedule "${result.name}". It will create ${result.creation_parameters?.placementBids ? '12' : '1'} campaign(s) at the next scheduled time.` });
                fetchSchedulesAndRules();
            } else {
                 setStatusMessage({ type: 'success', text: `Successfully created ${result.createdCampaigns.length} campaigns and associated rules.` });
            }
            setAsin('');
    ...
    const formatFrequency = (rule: CampaignCreationRule) => {
        const { frequency, creation_parameters } = rule;
        const bids = creation_parameters.placementBids;
        const bidString = bids ? `Bids: T:${bids.top}%|R:${bids.rest}%|P:${bids.product}%` : '';
        return `Every ${frequency.value} ${frequency.unit} at ${frequency.startTime} (UTC-7). ${bidString}`;
    };

    return (
        <div style={styles.container}>
            <h1 style={styles.title}>Create Auto Campaign Set</h1>
            
            <div style={styles.card}>
                <h2 style={styles.cardTitle}>Manage Scheduled Creations</h2>
                    ...
                    <div style={{overflowX: 'auto'}}>
                        <table style={styles.scheduleTable}>
                            <thead><tr>
                                ...
                                <th style={styles.scheduleTh}>Target ASIN</th><th style={styles.scheduleTh}>Details</th>
                                <th style={styles.scheduleTh}>Actions</th>
                            </tr></thead>
            ...
            <form style={styles.form} onSubmit={handleSubmit}>
                ...
                <div style={styles.card}>
                    ...
                    <div style={styles.biddingGrid}>
                        <div style={styles.formGroup}>
                            <label style={styles.label} htmlFor="placementTop">Top of search (%)</label>
                            <input id="placementTop" type="number" min="0" max="900" style={styles.input} value={placementBids.top} onChange={e => setPlacementBids(p => ({ ...p, top: parseInt(e.target.value, 10) || 0 }))} />
                        </div>
                        <div style={styles.formGroup}>
                            <label style={styles.label} htmlFor="placementRest">Rest of search (%)</label>
                            <input id="placementRest" type="number" min="0" max="900" style={styles.input} value={placementBids.rest} onChange={e => setPlacementBids(p => ({ ...p, rest: parseInt(e.target.value, 10) || 0 }))} />
                        </div>
                        <div style={styles.formGroup}>
                            <label style={styles.label} htmlFor="placementProduct">Product pages (%)</label>
                            <input id="placementProduct" type="number" min="0" max="900" style={styles.input} value={placementBids.product} onChange={e => setPlacementBids(p => ({ ...p, product: parseInt(e.target.value, 10) || 0 }))} />
                        </div>
                    </div>
                </div>
                ...
                <div style={styles.card}>
                    ...
                    <div style={styles.scheduleToggle}>
                        <label className="switch"><input type="checkbox" checked={isScheduling} onChange={e => setIsScheduling(e.target.checked)} /><span className="slider round"></span></label>
                    ...
                    {isScheduling && (
                        <div style={{ ...styles.scheduleGrid, marginTop: '20px' }}>
                            <div style={styles.formGroup}>
                                ...
                                <input id="scheduleName" style={styles.input} value={scheduleName} onChange={e => setScheduleName(e.target.value)} placeholder="e.g., Weekly New Auto Set for..." required={isScheduling} />
                            </div>
                            ...
                            <div style={styles.formGroup}><label style={styles.label}>Time (UTC-7)</label><input type="time" style={styles.input} value={frequency.startTime} onChange={e => setFrequency(p => ({ ...p, startTime: e.target.value }))} required={isScheduling} /></div>
                        </div>
                    )}
                </div>
                ...
                <div style={styles.buttonContainer}>
                    <button type="submit" style={loading.form ? { ...styles.button, ...styles.buttonDisabled } : styles.button} disabled={loading.form}>
                        {loading.form ? 'Submitting...' : isScheduling ? 'Save Schedule' : 'Create Campaign Set (12 Campaigns)'}
                    </button>
                </div>
            </form>
