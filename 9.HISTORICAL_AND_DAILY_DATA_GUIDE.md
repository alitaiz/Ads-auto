...
2.  **Tải Dữ liệu Báo cáo Sales & Traffic:**
    ...
    nohup npm run fetch:sales-traffic -- 2025-07-01 2025-08-17 &
    ```
...
2.  Một cửa sổ terminal mới sẽ hiện ra. Bên trong đó, chạy lệnh fetch:
    ```bash
    npm run fetch:sp-search-terms -- 2025-08-05 2025-09-04
    ```
...
2.  Trong phiên thứ hai này, chạy lệnh fetch còn lại:
    ```bash
    npm run fetch:sales-traffic -- 2025-07-01 2025-07-24
    ```
...
2.  **Thêm các dòng Cron Job:**
    ...
    0 1 * * * cd /var/www/Ads-auto && npm run fetch:sp-search-terms -- $(date -d "2 days ago" +\%Y-\%m-\%d) $(date -d "2 days ago" +\%Y-\%m-\%d) >> /var/www/Ads-auto/logs/fetch_sp_search_term.log 2>&1

    # Tự động lấy báo cáo Sales & Traffic hàng ngày lúc 1:05 AM cho dữ liệu của 2 ngày trước
    5 1 * * * cd /var/www/Ads-auto && npm run fetch:sales-traffic -- $(date -d "2 days ago" +\%Y-\%m-\%d) $(date -d "2 days ago" +\%Y-\%m-\%d) >> /var/www/Ads-auto/logs/fetch_sales_traffic.log 2>&1
    ```
