...
1.  **Di chuyển vào thư mục gốc của dự án** (ví dụ: `cd /var/www/Ads-auto`).
2.  **Khởi động ứng dụng Backend bằng PM2:**
    -   Chạy lệnh sau từ thư mục gốc. PM2 sẽ biết cách tìm `backend/server.js`.
    ```bash
    pm2 start backend/server.js --name "ppc-auto-backend"
    ```
