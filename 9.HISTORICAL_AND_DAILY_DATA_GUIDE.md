...
2.  Bên trong phiên `screen`, chạy các lệnh sau để yêu cầu báo cáo cho 60 ngày qua (thay đổi ngày tháng nếu cần):
    ...
    npm run request:sp-search-terms -- 2024-07-15 2024-09-12

    # Yêu cầu báo cáo Sponsored Brands
    npm run request:sb-search-terms -- 2024-07-15 2024-09-12

    # Yêu cầu báo cáo Sponsored Display
    npm run request:sd-targeting -- 2024-07-15 2024-09-12
    ```
...
2.  **Thêm các dòng Cron Job:**
    ...
    -   **CỰC KỲ QUAN TRỌNG:** Hãy thay đổi `/var/www/Ads-auto` thành đường dẫn **CHÍNH XÁC** đến thư mục dự án của bạn và đảm bảo dòng `PATH` là chính xác.
    ...
    # Giai đoạn 1: Yêu cầu báo cáo hàng ngày lúc 1:00 AM cho dữ liệu của 2 ngày trước
    0 1 * * * cd /var/www/Ads-auto && npm run request:sp-search-terms -- $(date -d "2 days ago" +\%Y-\%m-\%d) $(date -d "2 days ago" +\%Y-\%m-\%d) >> /var/www/Ads-auto/logs/cron_sp_search_term_request.log 2>&1
    5 1 * * * cd /var/www/Ads-auto && npm run request:sb-search-terms -- $(date -d "2 days ago" +\%Y-\%m-\%d) $(date -d "2 days ago" +\%Y-\%m-\%d) >> /var/www/Ads-auto/logs/cron_sb_search_term_request.log 2>&1
    8 1 * * * cd /var/www/Ads-auto && npm run request:sd-targeting -- $(date -d "2 days ago" +\%Y-\%m-\%d) $(date -d "2 days ago" +\%Y-\%m-\%d) >> /var/www/Ads-auto/logs/cron_sd_targeting_request.log 2>&1
    
    # Tự động lấy báo cáo Sales & Traffic hàng ngày lúc 1:10 AM cho dữ liệu của 2 ngày trước
    10 1 * * * cd /var/www/Ads-auto && npm run fetch:sales-traffic -- $(date -d "2 days ago" +\%Y-\%m-\%d) $(date -d "2 days ago" +\%Y-\%m-\%d) >> /var/www/Ads-auto/logs/cron_sales_traffic.log 2>&1
